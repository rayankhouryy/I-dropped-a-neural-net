...
\textit{Pairs.} \emph{Descendants:} the official RLHF model Llama-2-7B-chat, the Vicuna-7B instruction fine-tune, and CodeLlama-7B, a code-specialized continued-pretraining derivative of the base. \emph{Negatives:} four independently-trained checkpoints with no LLaMA-2 weight ancestry---Mistral-7B-v0.1, OpenLLaMA-7B, Yi-6B, and DeepSeek-R1-Distill-Llama-8B. The last is \emph{behaviorally} distilled from DeepSeek-R1 reasoning traces but \emph{weight-derived} from Meta's Llama-3.1-8B; its NON-DESCENDANT verdict w.r.t.\ LLaMA-2 demonstrates that the method tracks weight inheritance rather than behavioral pedigree, consistent with Section~\ref{sec:exp-baselines}.

\textit{OpenLLaMA: a same-architecture, from-scratch negative.} OpenLLaMA-7B is an open reproduction of the LLaMA architecture trained from scratch on different data. It shares LLaMA-2's exact residual shape ($d=4096$, $L=32$, $d_{\text{ff}}=11008$), tokenizer conventions, and RoPE setup, yet inherits none of its weights. It is therefore the strongest possible negative: architecture is held \emph{constant} and only weight lineage varies, so a configuration-level check cannot distinguish it from a genuine descendant---the verdict must come from the weights. Diagonal dominance scores it at $\mathcal{L}=4.5\times10^{-4}$, indistinguishable from the other independents.

\textit{Cross-model null.} The four negatives define a real cross-model null $\mu_0 = 4.2\times10^{-4}$, $\sigma_0 = 5.0\times10^{-5}$ (all four within $[3.5, 4.8]\times10^{-4}$). With only $n=4$ real independents this is a \emph{descriptive} separation check rather than a calibrated false-positive-rate estimate; we therefore report the raw $\mathcal{L}$ and the separation in orders of magnitude as the primary evidence, and defer a calibrated FPR to the larger checkpoint trees discussed below. The four negatives span three from-scratch pre-training lineages (Mistral, OpenLLaMA, Yi) plus a Llama-3 derivative, so the null is not dominated by any single family. \emph{Verdict rule for Table~\ref{tab:real-llm-lineage}:} we label a pair DESCENDANT if $\mathcal{L}$ exceeds a descriptive threshold placed an order of magnitude above the null ($\mathcal{L}>10^{-2}$) and NON-DESCENDANT otherwise; all seven pairs fall unambiguously on the expected side, with the nearest descendant (CodeLlama, $\mathcal{L}=0.336$) still more than two orders of magnitude above the null.
...
\midrule
\multicolumn{5}{@{}l}{\emph{Descendants (real fine-tunes)}} \\
Llama-2-7B-chat   & RLHF          & $0.995$ & $0.995$ & DESC. \\
Vicuna-7B         & instruct      & $0.996$ & $0.996$ & DESC. \\
CodeLlama-7B      & code PT       & $0.336$ & $0.414$ & DESC. \\
\midrule
\multicolumn{5}{@{}l}{\emph{Independents (cross-model null)}} \\
Mistral-7B-v0.1   & scratch  & $4\mathrm{e}{-}4$ & $0.158$ & NON. \\
OpenLLaMA-7B      & scratch$^\dagger$ & $4\mathrm{e}{-}4$ & $0.089$ & NON. \\
Yi-6B             & scratch  & $5\mathrm{e}{-}4$ & $0.122$ & NON. \\
DeepSeek-Distill-8B & L3.1 wts & $4\mathrm{e}{-}4$ & $0.162$ & NON. \\
\bottomrule
\end{tabular}
\caption{Real-LLM lineage validation on the public LLaMA-2 family (7 real pairs). All seven verdicts match expectation; DESC.\ / NON.\ abbreviate DESCENDANT / NON-DESCENDANT. $^\dagger$OpenLLaMA shares LLaMA-2's exact architecture ($d{=}4096$, $L{=}32$, $d_{\text{ff}}{=}11008$) but is trained from scratch, so its rejection cannot come from configuration. Diagonal dominance ($\mathcal{L}$) drives every independent to the $10^{-4}$ null while keeping descendants at $\ge 0.34$; weight cosine (W.\,cos) also separates but compresses the negatives only to ${\approx}0.13$. CodeLlama, a heavy code-specialized continued-pretraining fine-tune, is the weakest descendant ($\mathcal{L}=0.336$) yet still ${\sim}700\times$ above the null---graceful degradation with fine-tune intensity rather than a sharp threshold. All models share $d=4096$, $L=32$; branch products are $d\times d$ regardless of $d_{\text{ff}}$.}
\label{tab:real-llm-lineage}
\end{table}

\textit{Baseline separation.} Aggregated over the seven pairs, diagonal dominance separates descendants from independents by ${\approx}1850\times$ in mean score ($0.78$ vs.\ $4.2\times10^{-4}$), versus $6\times$ for weight cosine ($0.80$ vs.\ $0.13$); IPGuard, available only on the four-pair logit subset, gives $2\times$ ($0.25$ vs.\ $0.13$); aligned Frobenius and singular-value distance do not separate this real-model set (AUROC $0.667$ each), as their global spectral/Frobenius statistics are dominated by cross-model scale differences. This reproduces on real 7B weights the margin advantage established on the synthetic and CIFAR benchmarks: several methods clear AUROC on easy splits, but the residual-signature score preserves the largest descendant-vs-independent margin.

The set already spans a real RLHF model, a real instruction fine-tune, and a real continued-pretraining derivative (CodeLlama), tested against the strongest available negative---a same-architecture from-scratch model (OpenLLaMA) whose rejection cannot be explained by configuration alone. The remaining limitation is tree breadth: all descendants here derive from a single base (LLaMA-2-7B). Extending to additional real checkpoint trees (e.g.\ a Mistral or Qwen base with its own community fine-tunes) and to a larger negative set that supports a calibrated FPR is the natural next step.
