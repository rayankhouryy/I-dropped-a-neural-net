...
\begin{abstract}
With the growth of open-weight models, modern model supply chains increasingly rely on checkpoints that are fine-tuned, quantized, pruned, merged, and redistributed without reliable lineage metadata. Hashes fail after any weight change, metadata requires trust, and watermarks must be inserted before release. We study a post-hoc white-box question: given a reference checkpoint and a suspect checkpoint with matching residual architecture (same depth and width), can we determine whether they share weight-level lineage? We show that residual-network training itself induces an intrinsic fingerprint in residual-branch products. Correctly paired branch products develop diagonal-dominant structure, quantified by a trace/Frobenius score, that is absent at initialization, disappears under wrong factorization, and is not present in non-residual controls.

Across GPT-2, BERT, LLaMA-2, Mistral, Qwen2.5, DeepSeek-R1, ViT, Whisper, and ResNets, the fingerprint recovers block correspondences with 91–100\% accuracy. On controlled lineage benchmarks, the method achieves AUROC 1.000, detecting descendants after fine-tuning, pruning, quantization, and LoRA merging while rejecting independently trained and distilled non-descendants. The key advantage over simpler weight-space baselines is interpretable per-block correspondence and wider separation on real models. Activation-based CKA and decision-boundary IPGuard fail to reject distilled copies as non-descendants. This provides a passive, retroactive provenance signal for white-box models.
\end{abstract}
