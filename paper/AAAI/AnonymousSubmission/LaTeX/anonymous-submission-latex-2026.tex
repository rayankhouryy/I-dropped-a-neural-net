...
We find that trained residual architectures expose such a signal. Training couples a residual block's input and output projections so that their product exhibits diagonal dominance absent from mismatched or random pairings. We formalize this with the score $s(M)=|\mathrm{tr}(M)|/\|M\|_F$, prove a $\sqrt{d}$ upper bound and observe empirical growth $s \propto d^{0.87}$ across GPT-2 through LLaMA-2-13B, and build a two-level pipeline: diagonal dominance recovers block correspondences; centered residual signatures detect checkpoint lineage.
...
We compare against representation-similarity methods (CKA~\citep{kornblith2019similarity}, SVCCA~\citep{raghu2017svcca}), weight-space baselines (aligned Frobenius via Hungarian alignment, singular-value distance, weight cosine), and a decision-boundary baseline (IPGuard~\citep{cao2021ipguard}, adapted for regression via quantile-bin agreement) on the same 52-pair MLP benchmark used in Table~\ref{tab:survival} (Table~\ref{tab:baselines}). Five of seven methods reach AUROC$=$1.000; among them four are data-free, including ours. CKA drops to 0.829 because it confounds weight inheritance with behavioral similarity---it cannot distinguish a descendant fine-tuned on a new target from an independent model with similar activations. IPGuard collapses to chance (0.500): distilled non-descendants (which were trained to match the parent's outputs) score \emph{higher} than several true descendants, reproducing the structural limitation of decision-boundary methods on distillation. Our method matches the best baselines while requiring no probe data and remains robust under the conditions where the others fail.
...
Method & Type & AUROC \\
\midrule
\textbf{Diagonal Dominance (ours)} & Weight-space (data-free) & \textbf{1.000} \\
Aligned Frobenius & Weight-space (data-free) & 1.000 \\
Singular Value Dist.\ & Weight-space (data-free) & 1.000 \\
Weight Cosine & Weight-space (data-free) & 1.000 \\
SVCCA~\citep{raghu2017svcca} & Activation-space & 1.000 \\
CKA~\citep{kornblith2019similarity} & Activation-space & 0.829 \\
IPGuard~\citep{cao2021ipguard} (regr.) & Decision-boundary & 0.500 \\
\bottomrule
\end{tabular}
\caption{Strong-baseline comparison on the MLP lineage benchmark (52 pairs). Four data-free methods reach AUROC$=$1.000. CKA fails on fine-tunes to a new target; IPGuard's regression analogue ranks distilled non-descendants above true descendants. Full per-kind breakdown in Appendix~\ref{app:experiments}.}
\label{tab:baselines}
\end{table}
