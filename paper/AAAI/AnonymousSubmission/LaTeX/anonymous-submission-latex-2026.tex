...
\begin{abstract}
With the growth of open-weight models, modern model supply chains increasingly rely on checkpoints that are fine-tuned, quantized, pruned, merged, and redistributed without reliable lineage metadata. Existing solutions fall short: hashes fail after any weight change, metadata requires trust, and watermarks must be inserted before release. We study a post-hoc white-box verification problem: given a reference checkpoint and a suspect checkpoint with compatible residual architecture, can we determine whether the suspect inherits parameters from the reference after common checkpoint transformations? Our focus is weight-level provenance rather than behavioral similarity, making distillation a useful boundary case where functional imitation need not imply shared parameters. We show that residual-network training itself induces an intrinsic fingerprint in residual-branch products. Correctly paired branch products develop diagonal-dominant structure, quantified by a trace/Frobenius score, that is absent at initialization, disappears under wrong factorization, and is not present in non-residual controls.

Across GPT-2, BERT, LLaMA-2, Mistral, Qwen2.5, DeepSeek-R1, ViT, Whisper, and ResNets, the fingerprint recovers block correspondences with 91–100\% accuracy. On controlled lineage benchmarks, the method achieves perfect separation, detecting descendants after fine-tuning, pruning, quantization, and LoRA merging while rejecting independently trained and distilled non-descendants. The key advantage over simpler weight-space baselines is interpretable per-block correspondence and wider separation on real models. Activation-based (CKA) and decision-boundary-based (IPGuard) methods instead track functional similarity, causing them to assign descendant-like scores to distilled copies and making them unsuitable for weight-lineage verification. This provides a passive, retroactive provenance signal for white-box models.
\end{abstract}

\section{Introduction}

The open-weight ecosystem has transformed how neural networks are distributed and reused. When Meta released LLaMA~\citep{touvron2023llama}, the community produced instruction-tuned variants within weeks. Platforms like Hugging Face now host hundreds of thousands of model checkpoints~\citep{huggingface2024}, many of which are fine-tuned, quantized, or adapted via techniques like LoRA~\citep{hu2022lora} and model merging~\citep{ilharco2023editing}. This creates a \emph{model supply chain} where downstream checkpoints inherit weights from upstream models, but the lineage is often undocumented or intentionally obscured. Model reuse is now easy; reliable model provenance is not.

Existing provenance mechanisms fall short. Cryptographic hashes break after any weight modification. Metadata and model cards~\citep{mitchell2019model} require honest distributors and preserved records; they can be falsified or lost. Watermarks~\citep{uchida2017embedding,adi2018turning} must be inserted before release; they cannot be applied retroactively to already-deployed checkpoints, and no surveyed scheme proved robust against fine-tuning or pruning~\citep{lukas2022sok}. Behavioral tests that compare model outputs cannot distinguish weight inheritance from imitation: two models can behave similarly without sharing weights, especially after distillation~\citep{hinton2015distilling}. We need a passive, post-hoc signal that reads provenance from the weights themselves.
...
\caption{Training leaves traces in residual networks. (a) The residual branch product $M = W_{\mathrm{out}} W_{\mathrm{in}}$ for a block with skip connection. (b) Diagonal dominance emerges only in trained residual networks: random initialization shows no structure (2\% pair accuracy), trained ResNet shows strong diagonal (100\%), and trained PlainNet without skip connections shows no signal (3\%). (c) Lineage verification: descendants (fine-tuned, quantized, pruned, LoRA-merged) cluster near $\mathcal{L}=1$; non-descendants (independent, distilled) remain near $0$, enabling perfect separation.}
\label{fig:hero}
\end{figure*}
...
The fingerprint appears broadly across residual architectures. On public checkpoints (GPT-2~\citep{radford2019gpt2}, BERT~\citep{devlin2019bert}, LLaMA-2~\citep{touvron2023llama}, Mistral~\citep{jiang2023mistral}, Qwen2.5~\citep{yang2024qwen2}, DeepSeek-R1~\citep{deepseek2025r1}, ViT~\citep{dosovitskiy2021vit}, Whisper~\citep{radford2023whisper}, and ResNets~\citep{he2016deep}), correctly paired blocks recover correspondences with over 91\% accuracy (random-initialization baselines achieve below 9\%). Separation ratios grow from $35\times$ on GPT-2 to over $2{,}000\times$ on LLaMA-2-13B.

For provenance, we move from block correspondence to model-level verification. On controlled MLP and ResNet-18 benchmarks, descendants (checkpoints modified by fine-tuning~\citep{ouyang2022rlhf}, weight noise, pruning~\citep{han2015deep}, quantization~\citep{dettmers2022llmint8}, or LoRA merging~\citep{hu2022lora}) score high on our lineage metric, while non-descendants (independently trained models and distilled students~\citep{hinton2015distilling}) score low. This achieves AUROC$=$1.000 with 100\% true-positive rate at 1\% false-positive rate. Distilled students that replicate teacher outputs are correctly classified as non-descendants because they trained fresh weights (Figure~\ref{fig:hero}c). On the real-world LLaMA-2 family, the method correctly identifies base$\to$chat lineage and rejects an independent Llama-3-family checkpoint.

Activation-based methods like CKA~\citep{kornblith2019similarity} conflate behavioral imitation with weight inheritance. Decision-boundary fingerprints like IPGuard~\citep{cao2021ipguard} cannot distinguish true descendants from distilled copies. On our controlled benchmarks, simple weight-space baselines (weight cosine, aligned Frobenius) also achieve AUROC$=$1.000; the contribution is not aggregate performance but that the diagonal-dominance score (i) provides interpretable per-block correspondence as a by-product, and (ii) exhibits wider separation on real models. On the LLaMA-2 family, it achieves $1940\times$ separation (ratio between closest descendant and independent) versus $6\times$ for weight cosine.
