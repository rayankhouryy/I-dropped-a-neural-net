...
\bottomrule
\end{tabular}
\caption{AUROC under function-preserving laundering. P$=$permutation, D$_\mathrm{m/s}$$=$mild/strong rescaling, Lat.$=$latency (ms). D conditions apply only to MLP (GPT-2 uses GELU, which is not scale-invariant). Our method and Re-Basin maintain AUROC$=$1.0; raw baselines collapse. Ours is 2$\times$ faster on MLP, 76$\times$ faster on GPT-2. $^\dagger$Scores collapse 97\%; see Figure~\ref{fig:laundering-scores}.}
