...
\paragraph{Setup.}
We construct a controlled MLP benchmark (depth-16, hidden=48, input=16) with known ancestry. We train 2 root models independently from scratch on synthetic regression tasks. From each root, we derive 15 descendants via five transformation types (3 variants each): fine-tuning on the same task, fine-tuning on a shifted task, weight perturbation ($\sigma{=}0.02$), pruning (30\% sparsity), and quantization (8-bit). We also train 8 independent models per root (same architecture, different random seeds) and 3 distilled students per root (trained from scratch to mimic the root's outputs).

\paragraph{Evaluation protocol.}
Each (reference, suspect) pair receives a binary label: \emph{positive} if the suspect was derived from the reference's weights, \emph{negative} otherwise. This yields 30 positive pairs (root $\to$ descendant) and 22 negative pairs (11 independent models + 3 distilled students, per root $\times$ 2 roots), totaling 52 pairs. We report AUROC (threshold-free discrimination) and Gap-$Z$ (standardized margin between positive and negative score distributions).

\paragraph{Results.}
Table~\ref{tab:baselines} compares seven methods before any laundering. Activation-space methods (SVCCA, CKA) and decision-boundary methods (IPGuard) require probe samples and lack block-level interpretability. CKA achieves only AUROC$=$0.829, and IPGuard's negative Gap-$Z$ indicates it inverts the ranking on some pairs. SVCCA achieves AUROC$=$1.000 but requires forward passes through both models.
...
\bottomrule
\end{tabular}
\caption{Lineage detection baselines (52-pair MLP, no laundering). Data-free = no probe samples needed. Block Corr.\ = produces per-block similarity scores. Gap-$Z$ = standardized margin between related and unrelated scores.}
\label{tab:baselines}
\end{table*}

Weight-space methods---ours, aligned Frobenius, singular-value distance, and weight cosine---all achieve AUROC$=$1.000 on this clean benchmark. They require no probe data and operate directly on checkpoint weights. Weight cosine and aligned Frobenius show larger Gap-$Z$ margins ($+76.3$ and $+72.0$), but this advantage disappears under function-preserving reparameterization (Section~\ref{sec:exp-laundering}). Singular-value distance lacks block-level correspondence and fails on partial-inheritance scenarios (Appendix~\ref{app:harder-bench}).
