...
To test whether the fingerprint is specific to GPT-style causal decoders with GELU MLPs, we evaluate four further families: encoder-only \emph{BERT-base} \citep{devlin2019bert} (12 layers, bidirectional self-attention, MLM-pretrained, GELU); the SwiGLU+GQA decoder-only \emph{Mistral-7B} \citep{jiang2023mistral7b} (32 layers, sliding-window attention, grouped-query attention with $n_{\text{kv}}{=}8$); \emph{LLaMA-2-7B} \citep{touvron2023llama2} in both base and RLHF-tuned chat variants (32 layers, SwiGLU, RMSNorm, full multi-head attention with $n_{\text{kv}}{=}32$); and \emph{Qwen2.5-7B} \citep{qwen2.5} (28 layers, SwiGLU, RMSNorm, RoPE, with $Q/K/V$ biases and an aggressive 7{:}1 grouped-query ratio of $n_h{=}28$ to $n_{\text{kv}}{=}4$). For SwiGLU MLPs we score three architecture-aware factorizations---$W_{\text{down}}W_{\text{up}}$, $W_{\text{down}}W_{\text{gate}}$, and the joint stack $W_{\text{down}}[W_{\text{up}};W_{\text{gate}}]$---and expand the GQA $W_K,W_V$ projections to the query-head count before scoring on Mistral and Qwen2.5. The fingerprint generalizes broadly (Table~\ref{tab:transformer_family}); random-init baselines collapse to $\le\!9\%$ across families. This rules out three potential confounds at once: the signal is not specific to GELU activations (SwiGLU works across all three SwiGLU families), not specific to causal masking (bidirectional and sliding-window both work), and not specific to full attention (8{:}1 and 7{:}1 GQA both give AUC 1.000 on $W_OW_V$). The base$\to$chat comparison on LLaMA-2 is a direct robustness probe: RLHF fine-tuning preserves the fingerprint exactly on every path, with pair separations matching the base model to within $\pm 0.005$ on four of five paths; only $W_OW_V$ shows a measurable but non-fatal drop ($+0.141\to +0.036$).

\paragraph{Graceful degradation through factorization redundancy.}
Qwen2.5-7B is the first model in our sweep where a single sub-path falls below 100\%: $W_{\text{down}}W_{\text{gate}}$ recovers only 68\% of layers (AUC 0.940, pair sep $-0.025$). On the same model the complementary up-path $W_{\text{down}}W_{\text{up}}$ exhibits the strongest separation we observe anywhere in the cross-family table, sep $+2.869$, and the joint SwiGLU score $W_{\text{down}}[W_{\text{up}};W_{\text{gate}}]$ recovers to \textbf{100\%}. The fingerprint thus \emph{degrades gracefully}: when one architecture-aware product weakens, the joint factorization that combines complementary sub-paths still recovers the layer permutation exactly. This is consistent with the broader pattern across the family sweep---negative trace is \emph{sufficient} but not necessary, and global Hungarian assignment is robust enough to exploit diagonal mass even when row-wise dominance breaks down on individual paths.
...
\bottomrule
\end{tabular}
\caption{Cross-family transformer pairing across five families: encoder-only (BERT), SwiGLU+GQA+SWA decoder-only (Mistral), SwiGLU full-MHA decoder-only (LLaMA-2 base and chat), and SwiGLU+aggressive-GQA decoder-only (Qwen2.5). The fingerprint recovers all layers exactly on 23 of 24 evaluated paths. The single sub-100\% data point (Qwen2.5 $W_{\text{down}}W_{\text{gate}}$ at 68\%) is rescued to 100\% by the joint SwiGLU score, illustrating graceful degradation through factorization redundancy. The base$\to$chat comparison on LLaMA-2 shows RLHF fine-tuning preserves the fingerprint at 100\% pair accuracy across all paths. Random-init baselines for BERT and Mistral are averaged over 3 seeds; LLaMA-2 and Qwen2.5 random rows use 1 seed (laptop compute budget). GQA $W_K,W_V$ expanded to the query-head count before scoring on Mistral and Qwen2.5.}
