...
the central thesis is broader than originally stated:

\begin{quote}\itshape
Training induces a recoverable diagonal-dominance fingerprint in the
\emph{complete} residual-branch product, whatever its arity. For
two-factor branches (residual MLPs, BasicBlocks, transformer MLPs,
attention V/O and Q/K) the product is $W_{\mathrm{out}} W_{\mathrm{in}}$.
For three-factor branches (Bottleneck ResNets) the product is
$W_3 W_2 W_1$. The signal is training-induced (random-init baselines
remain at chance), persists under BatchNorm and LayerNorm, and is
invariant to the choice between channel-sum and center-tap spatial
projection.
\end{quote}
