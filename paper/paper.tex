...
\author{%
Rayan Khoury\thanks{Equal contribution.}\\
\texttt{rkhoury@alum.mit.edu}
\and
Aman Singh Thakur\footnotemark[1]\\
...
demonstrating that MLP sublayers of pretrained GPT-2 models exhibit
the same diagonal-dominance signal, achieving 100\% pairing accuracy
from 124M to 1.5B parameters.
...
\paragraph{Scope.} Our primary analysis focuses on residual MLPs
without normalization, across a discrete grid of depths and widths.
We then extend to transformer architectures (Section~\ref{sec:transformers}),
demonstrating that the MLP sublayers of pretrained GPT-2 models exhibit
the same diagonal-dominance signal with 100\% pairing accuracy.
Vision ResNets with BatchNorm remain out of scope and are discussed
...
\midrule
GPT-2 (random init) & 124M & 12 & 0\% & 50\% & --- & 0.12 \\
\bottomrule
\end{tabular}
\caption{MLP block pairing accuracy across the GPT-2 model family.
Diagonal-dominance achieves 100\% accuracy on all pretrained models.
The randomly initialized baseline confirms the signal is training-induced.}
\label{tab:gpt2}
\end{table}
...
\paragraph{Architectural scope.} Our ResNet experiments use MLP-style
blocks without normalization. Vision ResNets with BatchNorm exhibit
qualitatively different gradient dynamics~\cite{santurkar2018bn}: the
effective product becomes $W_{\mathrm{out}} \cdot \mathrm{diag}(\gamma) \cdot W_{\mathrm{in}}$
where $\gamma$ is the learned BatchNorm scale, potentially disrupting
the diagonal-dominance signal. This remains the most important untested
setting.

\paragraph{Transformer attention blocks.} While MLP sublayer pairing
works perfectly (Section~\ref{sec:transformers}), attention blocks
present a different challenge. The $W_V \to W_O$ factorization involves
intermediate attention weights $A = \mathrm{softmax}(QK^\top/\sqrt{d})$,
so the product $W_O W_V$ is not the full Jacobian of the attention
sublayer. Preliminary investigation suggests attention pairing may
require data-dependent scoring or alternative metrics.

\paragraph{Transformer ordering.} We demonstrate MLP \emph{pairing}
transfers to transformers but do not address layer \emph{ordering}.
The $\|W_{\mathrm{out}}\|_F$ heuristic has not been validated on
transformer blocks, and the interleaving of attention and MLP sublayers
may require new ordering strategies.
