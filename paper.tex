...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/fig_null_deepdive.png}
\caption{\textbf{Null model deep dive.} \emph{Top row:} the
diagonal-dominance matrix $d(i,j)$ at five training stages of a single
$24$-block ResNet, from untrained (left) to $300$ epochs (right). At
initialization there is no visible diagonal; by epoch $5$ the diagonal
is fully separated. \emph{(b) Identifiability trajectory:} pair
accuracy jumps from chance ($0.08$, only twice the $1/D=0.042$ floor)
at epoch $0$ to $1.00$ by epoch $5$; mean correct-pair score rises
monotonically from $0.19$ to $2.07$ while the mean incorrect-pair score
stays flat at $\approx 0.16$. \emph{(c) and (d):} densities of
correct-pair (red) vs.\ incorrect-pair (gray) $d$ scores at the
untrained and trained endpoints; the two distributions are
indistinguishable at init and disjoint after training. \emph{(e):} the
mean diag/off-diag gap, the key quantity in
Proposition~\ref{prop:margin}, grows from essentially zero to almost
two.}\label{fig:nullmodel}
\end{figure}
